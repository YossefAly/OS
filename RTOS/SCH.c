...
#include "RTOSFIFO.h"
struct {
	TCB_RTOS* OSTasks[100]; //Sch. Table
...
TCB_RTOS* Ready_QUEUE_Queue[100] ;
...
RTOS_Error_ID RTOS_init()
{
	...
	if (Queue_init(&Ready_QUEUE, Ready_QUEUE_Queue, 100) !=FIFO_NO_ERROR)
	{
		error += FIFO_NO_ERROR ;
	}
...
void MyRTOS_Create_TaskStack(TCB_RTOS* Tref)
{
	/*Task Frame
	 * ======
	 * XPSR
	 ...
	 */
	Tref->_C_PSP = Tref->_S_PSP_TASK ;

	Tref->_C_PSP-- ;
	*(Tref->_C_PSP) = 0x01000000;         //DUMMY_XPSR should T =1 to avoid BUS fault;//0x01000000

	Tref->_C_PSP-- ;
	*(Tref->_C_PSP) = (unsigned int)Tref->p_TaskEntry ; //PC

	Tref->_C_PSP-- ; //LR = 0xFFFFFFFD (EXC_RETURN)Return to thread with PSP
	*(Tref->_C_PSP)  = 0xFFFFFFFD ;
...
RTOS_Error_ID RTOS_Create_Task(TCB_RTOS* Tref)
{
	RTOS_Error_ID error = success ;
...
RTOS_Error_ID RTOS_Activate_Task (TCB_RTOS* Tref)
{
	Tref->TaskState = Waiting ;
	MYRTOS_OS_SVC_Set(SVC_Activatetask);
}
RTOS_Error_ID RTOS_Terminate_Task (TCB_RTOS* Tref)
{
	Tref->TaskState = Suspended ;
